...
args = {
    "project_id": "hello-generic-world-0407112858",
}

dag = DAG(
    "hello-generic-world-0407112858",
    default_args=args,
...
op_bb889c69_b23a_484e_8fb3_e69309f38a98 = KubernetesPodOperator(
    ...
    env_vars={
        ...
        "ELYRA_RUN_NAME": "hello-generic-world-0407112858-{{ ts_nodash }}",
    },
    in_cluster=True,
    config_file="None",
    dag=dag,
...
# Operator source: examples/pipelines/run-generic-pipelines-on-apache-airflow/load_data.py
op_b5a82b5e_f1e6_4ef4_b9d0_756b819c5247 = KubernetesPodOperator(
    name="data_ingest",
    namespace="admin",
    image="amancevice/pandas:1.4.1",
    cmds=["sh", "-c"],
    arguments=[
        "mkdir -p ./jupyter-work-dir/ && cd ./jupyter-work-dir/ && curl -H 'Cache-Control: no-cache' -L https://raw.githubusercontent.com/elyra-ai/elyra/master/elyra/airflow/bootstrapper.py --output bootstrapper.py && curl -H 'Cache-Control: no-cache' -L https://raw.githubusercontent.com/elyra-ai/elyra/master/etc/generic/requirements-elyra.txt --output requirements-elyra.txt && python3 -m pip install packaging && python3 -m pip freeze > requirements-current.txt && python3 bootstrapper.py --cos-endpoint http://localhost:9000 --cos-bucket elyra-test --cos-directory 'hello-generic-world-0407112858' --cos-dependencies-archive 'load_data-b5a82b5e-f1e6-4ef4-b9d0-756b819c5247.tar.gz' --file 'examples/pipelines/run-generic-pipelines-on-apache-airflow/load_data.py' "
    ],
    task_id="data_ingest",
    env_vars={
        "ELYRA_RUNTIME_ENV": "airflow",
...
op_8c96e288_4461_4d7e_8e0d_353c1fdb0c8c << op_b5a82b5e_f1e6_4ef4_b9d0_756b819c5247
